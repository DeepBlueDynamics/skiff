...
export function Vectors() {
  ...
  // Arrows stacked in the band above the coachroof (~2.2 m) and below the
  // boom (~4.2 m) — readable against the deck, never lost in the waves.
  return (
    <group position={[boat.position.x, boat.bobM, -boat.position.y]}>
      <Arrow directionDeg={boat.headingDeg} length={5} color="#f4f7f7" y={2.5} />
      <Arrow directionDeg={boat.cogDeg} length={Math.max(2, boat.sogMps * 2.2)} color="#ffcf5a" y={2.75} />
      <Arrow directionDeg={vectorToDeg(windGround)} length={Math.max(2.5, vectorMagnitude(windGround) * 0.55)} color="#7dd3fc" y={3.0} />
      <Arrow directionDeg={vectorToDeg(windWater)} length={Math.max(2.5, vectorMagnitude(windWater) * 0.55)} color="#38bdf8" y={3.25} />
      {settings.showCurrent && (
        <Arrow directionDeg={settings.currentToDeg} length={Math.max(1.5, settings.currentSpeedMps * 3.5)} color="#6ee7b7" y={3.5} />
      )}
    </group>
  );
}
